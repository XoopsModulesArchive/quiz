			...
			case 'Stat':
				...
				if (isset($id))
				{
					...
					$query = $xoopsDB->query(' SELECT * FROM ' . $xoopsDB->prefix('quiz_users').
						' WHERE id = '.$id.' ORDER BY score DESC LIMIT '.$eu.' , '.$limitUser);
					...
					if(isset($_GET['exp']) && $_GET['exp'] == 'on')
					{
						...
						$fp = fopen('../../../uploads/quiz.csv', 'w+b')
						or 
						redirect_header(XOOPS_URL.'/modules/quiz/admin/index.php?op=Stat', 3, '_QUIZ_OPEN_CSV_ERR'); 
...
';
			...
			default:
				...
				$menu->addItem('Preference', '../../system/admin.php?fct=preferences&amp;op=showmod&amp;mod='.$xoopsModule ->getVar('mid').
												'&amp;&confcat_id=1', '../images/menus/config.png', _QUIZ_PREFERENCE);
				...
				$tpl->assign('quiz_version', sprintf(_QUIZ_VERSION, $xoopsModule->getInfo('version').
				'('.$xoopsModule->getInfo('status').')',
																	$xoopsModule->getInfo('name')));
